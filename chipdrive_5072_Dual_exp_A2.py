...
class tmc5072():
    def __init__(self, clockfrequ=15000000, stepsPerRev=400, loglvl=logging.DEBUG, vmax=512000, amax=2200, a1=800, dmax=1700, d1=600, v1=25000, vstart=1, vstop=2, v2max=512000, a2max=2200, a21=800, d2max=1700, d21=600, v21=25000, v2start=1, v2stop=2):
        ...
        self.md=trinamicDriver.TrinamicDriver(clockfrequ=self.clockfrequ, datarate=1000000, pigp=self.pg,
                motordef=tmc5072regs.tmc5072, drvenpin=21, spiChannel=1, loglvl=loglvl )
    ...
    def rgoto2(self, targetpos, wait=True):
        self.md.enableOutput(True)
        self.md.writeInt('X2TARGET',int(targetpos))
        if wait:
            self.wait_reached2()

    def rgotonowait2(self, targetpos, wait=True):
        self.md.enableOutput(True)
        self.md.writeInt('X2TARGET',int(targetpos))
        
    def renergize2(self, wait=True):
        self.md.enableOutput(True)

    def rsoftenergize2(self, wait=True):
    ...
    def rmotorcurrent2(self, rihold2, rirun2, rihdelay2, wait=True):
        rihold2 = int(rihold2)
        rirun2 = int(rirun2) * 16**2
        rihdelay2 = int(rihdelay2) * 16**4
        rmotorcurrent2 = int(rihold2 + rirun2 + rihdelay2)
        self.md.writeInt('IHOLD_IRUN2',rmotorcurrent2)
        self.md.enableOutput(True)
            
    def rdenergize2(self, wait=True):
        self.md.enableOutput(False) 
